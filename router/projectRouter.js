...
router.get('/:id', validateProjectId, (req, res) => {
  projectdb
    ...
    .catch((err) => {
      ...
      res
        .status(500)
        .json({ message: 'There was a problem retrieving project by ID.' });
    });
});
...
router.get('/:id/actions', validateProjectId, (req, res) => {
  projectdb
    ...
    .catch((err) => {
      ...
      res
        .status(500)
        .json({ message: 'There was a problem getting project actions.' });
    });
});
...
router.put('/:id', validateProjectId, (req, res) => {
  projectdb
    ...
    .catch((err) => {
      ...
      res
        .status(500)
        .json({ message: 'There was an error updating that project.' });
    });
});
...
router.delete('/:id', validateProjectId, (req, res) => {
  projectdb
    ...
    .then((message) => {
      res.status(200).json({
        message: `The item with ID ${req.params.id} was successfully deleted.`,
      });
    })
    .catch((err) => {
      console.log(err);
